...
//Estrutura para dados de um livro:
typedef struct{
    char *name;
    double price;
}BOOK;

//Estruturas para dados de uma biblioteca:
typedef struct{
    BOOK **books;
    int size;
}LIBRARY;
...
char *readString(FILE *stream){
    ...
    int charCounter = 0;
    while(character < 33) character = fgetc(stream);
    do{
...
void mergeSort3(BOOK **books, int startPos, int endPos){
    ...
    while(i <= newEnd1) aux[l++] = books[i++];
    while(j <= newEnd2) aux[l++] = books[j++];
    while(k <= newEnd3) aux[l++] = books[k++];
