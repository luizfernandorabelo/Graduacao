...
int iterativeBinarySearch(NUMBER_ARRAY *numberArray, int key){
    ...
    while(min <= max){
        half = (min + max) / 2;
        if(numberArray->elements[half] == key) return half+1;
        else if(numberArray->elements[half] > key) max = half - 1;
        else min = half + 1;
    }
    return -1;
}
...
int recursiveBinarySearch(NUMBER_ARRAY *numberArray, int key, int min, int max){
    ...
    int half = (min + max) / 2;
    if(numberArray->elements[half] == key) return half+1;
    else if(numberArray->elements[half] > key) return recursiveBinarySearch(numberArray, key, min, half-1);
    else return recursiveBinarySearch(numberArray, key, half+1, max);
}
