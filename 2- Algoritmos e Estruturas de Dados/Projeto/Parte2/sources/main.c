...
int main(int argc, char *argv[]){
    ...
    while(TRUE){        
        ...
        if(operacao == INSERIR_SITE){
            ...
            servidor_inserir_palavra_chave_site(servidor, novaPalavra, codigoSite);
        ...
        else if(operacao == BUSCAR_SITES){
            char *palavraChave = ler_palavra_chave_site(stdin);
            boolean statusOperacao = servidor_buscar_sites(servidor, palavraChave);
            imprimir_termino_operacao(stdout, statusOperacao);
        }
        
        else if(operacao == SUGERIR_SITES){
            char *palavraChave = ler_palavra_chave_site(stdin);
            boolean statusOperacao = servidor_sugerir_sites(servidor, palavraChave);
            imprimir_termino_operacao(stdout, statusOperacao);
        }
